...
if (typeof process !== "undefined") {
    // If Node.js, require WebUSB library
    const USB = require("webusb").USB
    usb = new USB();
} else {
    // Otherwise, use browser implementation
    if (!navigator.usb)
...
class Board {
    ...
    enumerate_devices() {
        ...
        if (!usb) {
            console.log("error: WebUSB not supported");
            return;
        }
    ...
    async send_cmd(cmd, options={}) {
        ...
        if (!this.device || !this.device.opened) {
            this.logn("error: device not connected.\n", "red");
            this.logn("Use button to connect to a supported programmer.\n", "red");
