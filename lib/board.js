export default class Board {
    ...
    constructor(options={}) {
        ...
        this.logn = options.logn_method || console.log;
    ...
    async connect_device(dev) {
        ...
        this.set_device_info({
            ...
            specs: [
                <li>5,511 Logic Cells</li>,
                <li>16 MHz</li>,
                <li>32 GPIOs</li>,
                <li>4 inter-MCU IOs</li>
            ]
        });
    }
    ...
    enumerate_devices() {
        ...
        if (!navigator.usb) {
            console.log("error: browser does not support WebUSB");
            return;
        }
    ...
    async send_cmd(cmd, options={}) {
        ...
        if (!this.device || !this.device.opened) {
            document.getElementById("webusb-connect").style.backgroundColor = "red";
            document.getElementById("webusb-connect").innerHTML="web connect";
            this.log("error: device not connected.\n", "red");
            this.log("Use ", "red");
            this.log("web connect ", "black");
            this.log("button to connect to a supported programmer.\n", "red");
    ...
    async toggle_cpu_pin(port, bit) {
        // converts the above port/bit mapping to the true value
        // understood by the MCU.
        // FIXME: Need some extra edge cases to catch port/bit combinations
        //        not specified above.
        if (port == "A") {
            true_bit = (port == 0) ? 0 : bit-1;
        } else if (port == "B") {
            true_bit = 3;
        }

        try {
            this.log(`Toggling Programmer Port ${port} bit ${true_bit}...\n`, "blue");
            await this.at_verify("AFPA0", "Done", true_bit);
